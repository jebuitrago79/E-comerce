...
class ProductoCreate(ProductoBase):
    tenant_id: Optional[int] = 1  # default mientras conectas multitenant desde el front
...
def listar_productos(
    ...
    # Usa tu CRUD base (devuelve todos), y aplica filtros simples aquí
    productos = crud_obtener_productos(session)

    def ok(p: Producto) -> bool:
        if nombre and nombre.lower() not in (p.nombre or "").lower():
            return False
        if precio_min is not None and (p.precio or 0) < precio_min:
            return False
        if precio_max is not None and (p.precio or 0) > precio_max:
            return False
...
def actualizar_producto(producto_id: int, data: ProductoUpdate, session: Session = Depends(get_session)):
    try:
        ...
        if not p:
            raise HTTPException(status_code=404, detail="Producto no encontrado")
        p = crud_actualizar_producto(session, producto_id, nuevos)
        return p
    except Exception as e:
...
def eliminar_producto(producto_id: int, session: Session = Depends(get_session)):
    p = crud_eliminar_producto(session, producto_id)
    if not p:
